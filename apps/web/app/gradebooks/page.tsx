...
const TEACHER_USER_ID = "77777777-7777-4777-8777-777777777772";
...
const SECTION_SUBJECT_OPTIONS = [
  {
    ...
    label: "6ème A · Mathématiques",
  },
  {
    id: "8bbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbb2",
    label: "6ème A · Français",
  },
  {
    id: "8bbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbb3",
    label: "6ème B · Mathématiques",
  },
  {
    id: "8bbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbb4",
    label: "6ème B · Anglais",
  },
  {
    id: "8bbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbb5",
    label: "5ème A · Mathématiques",
  },
  {
    id: "8bbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbb6",
    label: "5ème A · Sciences",
  },
] as const;

const GRADING_PERIOD_OPTIONS = [
  {
...
type AttendanceSectionMap = Record<string, string>;

const SECTION_BY_SECTION_SUBJECT: AttendanceSectionMap = {
...
export default function GradebooksPage() {
  ...
  const currentSectionId = useMemo(
    () => SECTION_BY_SECTION_SUBJECT[sectionSubjectId],
    [sectionSubjectId],
  );
  ...
  async function handleSaveScores(e: FormEvent) {
    ...
    try {
      await apiPost("/assessment-scores/bulk-upsert", {
        ...
        enteredByUserId: TEACHER_USER_ID,
  ...
  async function handleSubmitGradebook() {
    ...
    try {
      const updated = await apiPost<Gradebook>("/gradebooks/submit", {
        gradebookId: gradebook.id,
        submittedByUserId: TEACHER_USER_ID,
      });
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 md:grid-cols-3">
          <div>
            ...
            >
              {SECTION_SUBJECT_OPTIONS.map((item) => (
          ...
          <div>
            ...
            >
              {GRADING_PERIOD_OPTIONS.map((item) => (
                <option key={item.id} value={item.id}>
                  {item.label}
                </option>
              ))}
