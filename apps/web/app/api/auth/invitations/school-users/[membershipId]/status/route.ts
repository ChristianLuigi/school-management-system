...
export async function PATCH(request: NextRequest,{params}:{params:Promise<{membershipId:string}>}){
 ...
 if(!token)return NextResponse.json({message:"Authentication required."},{status:401});const {membershipId}=await params;
 const upstream=await fetch(`${API_BASE_URL}/auth/invitations/school-users/${encodeURIComponent(membershipId)}/status`,{method:"PATCH",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:await request.text(),cache:"no-store"});
