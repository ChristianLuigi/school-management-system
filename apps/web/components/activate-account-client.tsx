...
type Invitation = {
  valid: boolean; schoolName: string; emailMasked: string; roleCode: string;
  locale: "fr" | "en"; firstName: string | null; lastName: string | null;
  expiresAt: string; accountAlreadyExists: boolean;
};
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch("/api/auth/invitations/accept", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ token, firstName, lastName, password, passwordConfirmation }),
      });
      const body = await response.json().catch(() => null);
      if (!response.ok) {
...
function Shell({ children }: { children: React.ReactNode }) {
  return <main className="flex min-h-screen items-center justify-center bg-slate-50 p-6"><div className="max-w-md rounded-2xl border bg-white p-8">{children}</div></main>;
}

function Field({ label, value, onChange, autoComplete }: {
  label: string; value: string; onChange: (value: string) => void; autoComplete: string;
}) {
  return <label className="text-sm"><span className="font-medium">{label}</span><input required autoComplete={autoComplete}
...
function PasswordField({ label, value, onChange }: {
  ...
  return <label className="mt-4 block text-sm"><span className="font-medium">{label}</span><input required type="password"
    autoComplete="new-password" minLength={15} maxLength={128} value={value}
