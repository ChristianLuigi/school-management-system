...
export class AuthService {
  ...
  private absoluteHours() {
    return this.numberSetting('AUTH_SESSION_ABSOLUTE_HOURS', 12);
  }
  private idleMinutes() {
    return this.numberSetting('AUTH_SESSION_IDLE_MINUTES', 30);
  }
  private maxFailures() {
    return this.numberSetting('AUTH_MAX_FAILED_LOGINS', 5, 3);
  }
  private lockMinutes() {
    return this.numberSetting('AUTH_ACCOUNT_LOCK_MINUTES', 15);
  }
  ...
  async login(dto: LoginDto) {
    ...
    const idleExpiresAt = new Date(
      Math.min(expiresAt.getTime(), now + this.idleMinutes() * 60_000),
    );
    ...
    if (user.password_hash && this.passwords.needsRehash(user.password_hash)) {
      const replacement = await this.passwords.hash(
        dto.password.normalize('NFC'),
      );
      await this.db.query(
  ...
  async inspectPasswordReset(rawToken: string) {
    ...
    return {
      valid: true,
      emailMasked: this.maskEmail(reset.email_original),
  ...
  async confirmPasswordReset(dto: ConfirmPasswordResetDto) {
    ...
    const result = await this.db.withTransaction(async (client) => {
      ...
      const password = this.passwords.validate(dto.password, [
        reset.first_name ?? '',
        reset.last_name ?? '',
        reset.email_normalized.split('@')[0],
        reset.school_names ?? '',
      ]);
      if (
        reset.password_hash &&
        (await this.passwords.verify(reset.password_hash, password))
      ) {
        throw new BadRequestException(
      ...
      return {
        resetRequestId: reset.reset_id,
        userId: reset.user_id,
        email: reset.email_original,
  ...
  async requireSession(authorization?: string) {
    ...
    const result = await this.db.query<{
      ...
      user_id: string;
      authentication_version: number;
      current_authentication_version: number;
      expires_at: Date;
    ...
    const invalid =
      ...
      session.authentication_version !==
        session.current_authentication_version ||
    ...
    const roles = await this.db.query<{ school_id: string; role_code: string }>(
      ...
      WHERE sm.user_id=$1 AND sm.membership_status='ACTIVE' AND sm.deleted_at IS NULL AND smr.deleted_at IS NULL
        AND (
          smr.role::TEXT NOT IN ('SCHOOL_ADMIN', 'TEACHER', 'FINANCE_ADMIN')
          OR EXISTS (
            SELECT 1
    ...
    return {
      ...
      email: session.email_original,
      user_email: session.email_original,
      user_status: session.account_status,
      expires_at: session.expires_at.toISOString(),
      revoked_at: null,
      school_roles: roles.rows.map((role) => ({
