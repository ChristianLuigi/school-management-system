CREATE TABLE IF NOT EXISTS school_academic_structure_settings (
  school_id UUID PRIMARY KEY REFERENCES schools(id),
  include_kindergarten BOOLEAN NOT NULL DEFAULT FALSE,
  include_primary BOOLEAN NOT NULL DEFAULT FALSE,
  include_secondary BOOLEAN NOT NULL DEFAULT FALSE,
  structure_mode TEXT NOT NULL DEFAULT 'HAITIAN_STANDARD',
...
CREATE OR REPLACE FUNCTION ensure_haitian_academic_structure_for_divisions(
  p_school_id UUID,
  p_divisions TEXT[]
)
RETURNS VOID
...
DECLARE
  ...
  v_section_id UUID;
BEGIN
  IF p_divisions IS NULL OR array_length(p_divisions, 1) IS NULL THEN
    RAISE EXCEPTION 'At least one academic division must be selected.';
  END IF;
  ...
  FOR rec IN
    ...
    WHERE division = ANY(p_divisions)
  LOOP
    SELECT id
